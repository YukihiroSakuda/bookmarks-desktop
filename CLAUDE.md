...
- `src/lib/uiLanguage.ts` — the en/ja choice for the Help and Settings dialogs (localStorage `ui_lang`, like the theme; `en` by default). Owned by `BookmarkHeader`, which renders both dialogs and passes it down. Settings → Appearance is the canonical switch; Help repeats it as an EN/JA pair in its own header, because that is where the long prose is and a reader who opened the wrong language should not have to leave to fix it. Both write the same value. The rest of the app stays English per the conventions below
