...
3. **Adoption:** ブックマーク一覧で選択モードに入り、6件をチェックして「Add to group」→ グループ名「案件A 朝イチ」を作成
...
2. 一覧で対象カードの「⋯」→「Add to group」→ 既存グループ「案件B」を選択
...
#### Feature 2: 複数選択によるグループへの一括追加

- **User Story:** ユーザーとして、一覧から複数のブックマークをまとめて選び、1操作でグループに入れたい。そうすれば、グループ作成が数十秒で終わる
- **Acceptance Criteria:**
  - [ ] Given ブックマーク一覧が表示されている, When 選択モードに入る, Then 各カードにチェックボックスが表示され、通常のクリックでの起動は無効になる
  - [ ] Given 選択モードで3件を選択した, When 「Add to group」を押して既存グループを選ぶ, Then 3件がそのグループの末尾に追加される
  - [ ] Given 選択モードで3件を選択した, When 「Add to group」→「新規グループ」を選び名前を入力する, Then その3件を含む新しいグループが作成される
  - [ ] Given すでにグループに所属している1件を含む3件を選択した, When 同じグループに追加する, Then 重複追加されず、所属していなかった2件だけが追加される
  - [ ] Given 選択モードである, When Escape を押す, Then 選択がすべて解除され、選択モードを抜ける
...
| `add_bookmarks_to_group` | 複数選択からの追加（重複はスキップ） |
...
| `src/hooks/useBookmarkSelection.ts` | 選択モードの状態。Escape 解除は既存 `useKeyboardShortcuts` に接続 |
...
| `src/components/BookmarkCard.tsx` | 選択モード時のチェックボックス表示を追加 |
...
| **P3: 選択モード** | `useBookmarkSelection`、カードのチェックボックス、「Add to group」 | 一覧から複数選んで数十秒でグループが作れる |
...
2. 選択モードへの入り方（専用ボタン / `Ctrl` + クリック / 長押し）。既存の並び替えモードの入り方と揃えるのが自然
