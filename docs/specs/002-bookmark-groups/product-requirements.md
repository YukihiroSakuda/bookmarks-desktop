...
| ✅ **P5: 仕上げ** | グループ内並び替え、グループカードの並び替え、ショートカット | Should Have を満たす |
...
### 実装で決めたこと（設計時に未決だったもの）

- **起動の確認しきい値は1箇所で判定する。** 当初は `GroupCard` 内に確認ダイアログを置いたが、ショートカット起動がそれを迂回してしまうため `page.tsx` に集約した。カードのクリックとショートカットが同じしきい値に従う
- **ショートカットの名前空間はブックマークと共有。** グループのフォームは既存のブックマーク・グループ両方との重複を拒否する。ただしブックマーク側のフォームはグループを見ないため、後から作ったブックマークが衝突しうる。`useGroupHotkeys` はその場合ブックマークを優先し、両方が同時に発火することを防ぐ
- **メンバーの並び替えは未解決 ID を保持する。** 並び替えは membership の全置換なので、ブックマーク一覧の読み込みが終わっていない等で行として描画できなかった ID は末尾に引き継ぐ（削除しない）
- **グループも `appCache` に載せる。** 起動直後に Groups ビューが空から描き直されないようにする。グループを持たない古いキャッシュのために optional
