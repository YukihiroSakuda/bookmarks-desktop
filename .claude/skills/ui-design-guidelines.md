...
- **タグの色（唯一の多色の例外）**: ユーザーが Tag Manager で明示的に色を設定したタグのみ、青以外の色を使ってよい。パレットと具体的なクラスは `src/lib/tagColors.ts` に集約する（8色固定、任意の色は不可）
  ...
  - 色を設定していないタグは従来どおり（非選択=`bg-secondary` / 選択=青）。既定は「色なし」
